!> MOM6 Coriolis and Momentum Advection Module (ORIGINAL - 2D per-layer version)
!!
!! Original implementation with sequential k-loop and 2D work arrays.
!! Kept for A/B performance comparison against the 3D parallel version.
!!
!! Original code from: src/core/MOM_CoriolisAdv.F90
!!
module mom6_coriolis_orig
    use iso_fortran_env, only: dp => real64
    use mom6_types, only: ocean_grid_type, verticalGrid_type
    implicit none
    private

    public :: CorAdCalc_orig, coriolis_init_orig, coriolis_end_orig
    public :: coriolis_CS_orig
    ...
    !> Control structure for Coriolis solver (original 2D work arrays)
    type :: coriolis_CS_orig
...
contains
    ...
    subroutine coriolis_init_orig(CS, G, scheme)
        type(coriolis_CS_orig), intent(inout) :: CS
    ...
    end subroutine coriolis_init_orig

    !> Finalize the Coriolis solver
    subroutine coriolis_end_orig(CS)
        type(coriolis_CS_orig), intent(inout) :: CS
    ...
    end subroutine coriolis_end_orig

    !> Calculate Coriolis and momentum advection accelerations (original per-layer version)
    subroutine CorAdCalc_orig(u, v, h, uh, vh, CAu, CAv, G, GV, CS)
        type(ocean_grid_type), intent(in) :: G
    ...
    end subroutine CorAdCalc_orig

end module mom6_coriolis_orig

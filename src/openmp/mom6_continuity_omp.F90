...
contains
    ...
    subroutine continuity_init(CS, G, GV, uhbt, u_cor, du_cor, por_face_areaU, visc_rem_u)
        ...
        !$omp target enter data map(to: uhbt, por_face_areaU, visc_rem_u)
        !$omp target enter data map(alloc: u_cor, du_cor)
        !$omp target enter data map(alloc: CS%duhdu, CS%du, CS%du_max_CFL, CS%du_min_CFL)
        !$omp target enter data map(alloc: CS%duhdu_tot_0, CS%uh_tot_0, CS%visc_rem_max)
    ...
    subroutine continuity_end(CS, uhbt, u_cor, du_cor, por_face_areaU, visc_rem_u)
        ...
        !$omp target exit data map(delete: CS%duhdu, CS%du, CS%du_max_CFL, CS%du_min_CFL)
        !$omp target exit data map(delete: CS%duhdu_tot_0, CS%uh_tot_0, CS%visc_rem_max)
        !$omp target exit data map(delete: uhbt, por_face_areaU, visc_rem_u)
        !$omp target exit data map(delete: u_cor, du_cor)
  ...
  !! Updates layer thickness h from hin using velocities u, v
    subroutine continuity_PPM(u, hin, h, uh, dt, G, GV, CS, por_face_areaU, uhbt, visc_rem_u, u_cor, BT_cont, du_cor)
        ...
        !$omp target data map(alloc: h_W, h_E)
...
!> GPU kernel: Compute uh and duhdu for all layers — replaces per-j zonal_flux_layer calls
    subroutine zonal_flux_layer_3d(u, h_in, h_W, h_E, uh, CS, visc_rem_u, &
        ...
        !$omp target teams distribute parallel do collapse(3) private(CFL, curv_3, h_marg, visc_rem_val)
        do k = 1, nz
            do j = jsh, jeh
                do I = ish - 1, ieh
                    if (use_visc_rem) then
...
!> GPU kernel: Compute CFL limits for du correction
    subroutine zonal_CFL_limits_gpu(u, CS, visc_rem_u, dt, G, GV, CFL_dt, I_dt, use_visc_rem)
        ...
        !$omp target teams distribute parallel do collapse(2) &
        !$omp   private(I_vrm, dx_W, dx_E, du_lim, vrm_k)
        do j = jsh, jeh
            do I = ish - 1, ieh
...
!> 3D PPM reconstruction — single collapse(3) kernel instead of nz separate kernel launches.
    subroutine PPM_reconstruction_x_3d(h_in, h_W, h_E, G, GV, h_min, monotonic, simple_2nd)
        ...
        if (simple_2nd) then
            !$omp target teams distribute parallel do collapse(3) private(h_im1, h_ip1)
            do k = 1, nz
                do j = jsl, jel
                    do i = isl, iel
            ...
            !$omp target teams distribute parallel do collapse(3) &
            !$omp   private(h_im1, h_ip1, dMx, dMn, slp_im1, slp_i, slp_ip1)
            do k = 1, nz
                do j = jsl, jel
                    do i = isl, iel
        ...
        if (monotonic) then
            ...
            !$omp target teams distribute parallel do collapse(3) private(curv, dh, scale_val)
            do k = 1, nz
                do j = jsl, jel
                    do i = isl, iel
...
!   endif
        ...
        if (simple_2nd) then
            !$omp target teams distribute parallel do collapse(2) private(h_im1, h_ip1)
            do j = jsl, jel
                do i = isl, iel
            ...
            !$omp target data map(alloc: slp)
            !$omp target teams distribute parallel do collapse(2) private(dMx, dMn)
            do j = jsl, jel
                do i = isl - 1, iel + 1
                    if ((G%mask2dT(i - 1, j)*G%mask2dT(i, j)*G%mask2dT(i + 1, j)) == 0.0_dp) then
                        slp(i, j) = 0.0_dp
...
! Local variables
        ...
        !$omp target teams distribute parallel do collapse(2) private(curv, dh, scale)
        do j = jis, jie
            do i = iis, iie
   ...
   !! according to the monotonic prescription of Colella and Woodward, 1984.
    subroutine PPM_limit_CW84(h_in, h_L, h_R, G, iis, iie, jis, jie)
        ...
        !$omp target teams distribute parallel do collapse(2) private(h_i, RLdiff, RLdiff2, RLmean, FunFac)
        do j = jis, jie
            do i = iis, iie
...
end module mom6_continuity_omp

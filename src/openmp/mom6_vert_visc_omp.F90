...
contains
    ...
    subroutine vert_visc_init(CS, G, GV, Kv, Kv_ml, Kv_extra_bbl, Hmix, Hbbl, harmonic_visc)
        ...
        ! Compute total bytes: 2*(nz+1) + 4*nz 3D arrays + 2 2D arrays, all real(dp)
        CS%nbytes = int(G%ied - G%isd + 1, int64) * int(G%jed - G%jsd + 1, int64) &
            * (2_int64 * int(nz + 1, int64) + 4_int64 * int(nz, int64) + 2_int64) * 8_int64
    ...
    subroutine vert_visc_coef(u, v, h, CS, G, GV)
        ...
        !$omp target data map(to: u, v, h)

        ! --- U-points ---
        !$omp target teams distribute parallel do collapse(2) &
        !$omp   private(hvel, z_i, h_harm, h_arith, h_delta, z2, botfn, z_top, Kv_tot, topfn, h_shear, k, K2)
        do j=js,je
        do i=is - 1,ie
    ...
    subroutine vert_visc_remnant(dt, CS, G, GV, visc)
        ...
        ! --- U-points ---
        !$omp target teams distribute parallel do collapse(2) &
        !$omp   private(c1, b1, d1, b_denom_1, Ray, k)
        do j=js,je
        do i=is - 1,ie
            if (G%mask2dCu(i, j) > 0.0_dp) then
    ...
    subroutine vert_visc_apply(u, v, h, dt, CS, G, GV, forces, visc)
        ...
        !$omp target data map(tofrom: u, v) map(to: h)

        ! --- Apply to u-velocity ---
        !$omp target teams distribute parallel do collapse(2) &
        !$omp   private(c1, sfc_stress, Ray, b1, d1, b_denom_1, k)
        do j=js,je
        do i=is - 1,ie
            if (G%mask2dCu(i, j) > 0.0_dp) then
    ...
    subroutine vert_visc_coef_apply(u, v, h, dt, CS, G, GV, forces, visc)
        ...
        !$omp target teams distribute parallel do collapse(2) &
        !$omp   private(hvel, z_i, h_harm, h_arith, h_delta, z2, botfn, z_top, Kv_tot, topfn, h_shear, &
        !$omp           sfc_stress, Ray, b1, d1, b_denom_1, a_k, a_k1, h_eff, k)
        do j=js,je
        do i=is - 1,ie
            if (G%mask2dCu(i, j) > 0.0_dp) then

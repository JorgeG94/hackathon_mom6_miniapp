...
contains
    ...
    subroutine hor_visc_init(CS, G, GV, Kh, Ah, Laplacian, biharmonic, &
        ...
        !$acc enter data copyin(CS%Kh_bg_xx, CS%Kh_bg_xy)
        !$acc enter data copyin(CS%Kh_Max_xx, CS%Kh_Max_xy)
        ...
        !$acc enter data create(CS%str_xx, CS%str_xy)

        if (.not. CS%use_fused_kernels) then
            ! Full mode: transfer all intermediate work arrays
            !$acc enter data create(CS%dudx, CS%dvdy, CS%dvdx, CS%dudy)
            !$acc enter data create(CS%sh_xx, CS%sh_xy)
            !$acc enter data create(CS%h_u, CS%h_v, CS%hq)
            !$acc enter data create(CS%Kh)
        end if

        if (CS%biharmonic) then
            !$acc enter data copyin(CS%Ah_bg_xx, CS%Ah_bg_xy)
            !$acc enter data copyin(CS%Ah_Max_xx, CS%Ah_Max_xy)
            ...
            !$acc enter data create(CS%Ah, CS%Del2u, CS%Del2v)
        end if
        if (CS%Smagorinsky_Kh .or. CS%Smagorinsky_Ah) then
            !$acc enter data copyin(CS%Laplac2_const_xx, CS%Laplac2_const_xy)
            !$acc enter data create(CS%Shear_mag)
        end if
        if (CS%Leith_Kh .or. CS%Leith_Ah) then
    ...
    subroutine hor_visc_end(CS)
        ...
        !$acc exit data delete(CS%Kh_bg_xx, CS%Kh_bg_xy)
        !$acc exit data delete(CS%Kh_Max_xx, CS%Kh_Max_xy)
        ...
        !$acc exit data delete(CS%str_xx, CS%str_xy)
        ...
        if (allocated(CS%dudx)) then
            ...
            !$acc exit data delete(CS%sh_xx, CS%sh_xy)
            !$acc exit data delete(CS%h_u, CS%h_v, CS%hq)
            !$acc exit data delete(CS%Kh)
        end if

        ! Conditional exit data for feature-specific arrays
        if (allocated(CS%Ah_bg_xx)) then
            !$acc exit data delete(CS%Ah_bg_xx, CS%Ah_bg_xy)
            !$acc exit data delete(CS%Ah_Max_xx, CS%Ah_Max_xy)
   ...
   !!
    ...
    subroutine hor_visc_fused(u, v, h, diffu, diffv, G, GV, CS)
        ...
        !=====================================================================
        !$acc parallel loop collapse(3) private(dudx_l, dvdy_l, sh_xx_l)
        do k = 1, nz
            do j = Jsq, Jeq + 1
                do i = Isq, Ieq + 1
        ...
        !=====================================================================
        !$acc parallel loop collapse(3) private(dvdx_l, dudy_l, sh_xy_l, hq_l)
        do k = 1, nz
            do J = Jsq, Jeq
                do I = Isq, Ieq
                    ! Velocity gradients (inline, register)
        ...
        !=====================================================================
        !$acc parallel loop collapse(3) private(h_u_l)
        do k = 1, nz
            do j = js, je
                do I = Isq, Ieq
        ...
        !=====================================================================
        !$acc parallel loop collapse(3) private(h_v_l)
        do k = 1, nz
            do J = Jsq, Jeq
                do i = is, ie

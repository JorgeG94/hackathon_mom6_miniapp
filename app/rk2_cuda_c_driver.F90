!> Unified RK2 driver using CUDA C kernels for Coriolis,
!! CUDA Fortran for remaining physics modules.
!!
!! This driver is identical to rk2_cuda_driver.F90 except:
!!   - Coriolis uses mom6_coriolis_cuda_c (CUDA C kernels via iso_c_binding)
!!   - Other modules still use CUDA Fortran
!!
!! Usage: rk2_cuda_c_driver [ni] [nj] [nk] [niter] [bt_nsteps]
...
program rk2_cuda_c_driver
    ...
    ! CUDA C Coriolis (portable module — no cudafor dependency)
    ...
    type(coriolis_CS_cuda_c) :: cor_CS    ! <-- CUDA C version
    type(barotropic_CS_cuda) :: bt_CS_cuda
    type(vert_visc_CS_cuda)  :: visc_CS
    type(hor_visc_CS_cuda)   :: hvisc_CS

    ! Shared GPU workspace pool (for non-coriolis modules)
    ...
    print '(A)', '================================================================'
    print '(A)', 'MOM6 Split RK2 — CUDA C Kernels Driver'
    print '(A)', '================================================================'
    ...
    ! --- Coriolis CUDA C init (portable, via iso_c_binding) ---
    ...
    ! --- Horizontal viscosity CUDA init ---
    ...
    ! Initialize shared GPU workspace pool (for non-coriolis modules)
    ...
    ! Coriolis warmup — pass device pointers via c_devloc
    call CorAdCalc_cuda_c( &
        c_devloc(u_d(G%isd, G%jsd, 1)), c_devloc(v_d(G%isd, G%jsd, 1)), &
        c_devloc(h_d(G%isd, G%jsd, 1)), c_devloc(uh_d(G%isd, G%jsd, 1)), &
        c_devloc(vh_d(G%isd, G%jsd, 1)), &
        c_devloc(CAu_d(G%isd, G%jsd, 1)), c_devloc(CAv_d(G%isd, G%jsd, 1)), &
        cor_CS)
    ...
    !=========================================================================
    print '(A)', 'Running split RK2 time-stepping (CUDA C)...'
    print '(A)', ''

    t_coriolis = 0.0_dp; t_barotropic = 0.0_dp; t_continuity = 0.0_dp
    t_vert_visc = 0.0_dp; t_hor_visc = 0.0_dp; t_compute = 0.0_dp
    ...
    do iter = 1, niter
        ...
        call hor_visc_cuda(u_d, v_d, h_d, diffu_d, diffv_d, hvisc_CS, nk, ws)
        t_end = omp_get_wtime(); t_hor_visc = t_hor_visc + (t_end - t_start)
        ...
        call CorAdCalc_cuda_c( &
            ...
            c_devloc(CAu_d(G%isd, G%jsd, 1)), c_devloc(CAv_d(G%isd, G%jsd, 1)), &
            cor_CS)
        t_end = omp_get_wtime(); t_coriolis = t_coriolis + (t_end - t_start)
        ...
        t_end = omp_get_wtime(); t_vert_visc = t_vert_visc + (t_end - t_start)
        call profiler_stop("VertVisc")

        call profiler_start("Barotropic")
        t_start = omp_get_wtime()
        call btstep_cuda(eta_d, ubt_d, vbt_d, ubt_av_d, vbt_av_d, eta_av_d, bt_CS_cuda)
        t_end = omp_get_wtime(); t_barotropic = t_barotropic + (t_end - t_start)
        ...
        call continuity_PPM_cuda(up_d, h0_d, h_d, uh_d, dt, cont_CS, ws, &
                                 uhbt_cont_d, BT_cont_cuda)
        t_end = omp_get_wtime(); t_continuity = t_continuity + (t_end - t_start)
        ...
        t_start = omp_get_wtime()
        call CorAdCalc_cuda_c( &
            c_devloc(up_d(G%isd, G%jsd, 1)), c_devloc(vp_d(G%isd, G%jsd, 1)), &
            c_devloc(h_d(G%isd, G%jsd, 1)), c_devloc(uh_d(G%isd, G%jsd, 1)), &
            c_devloc(vh_d(G%isd, G%jsd, 1)), &
            c_devloc(CAu_d(G%isd, G%jsd, 1)), c_devloc(CAv_d(G%isd, G%jsd, 1)), &
            cor_CS)
    ...
    call profiler_report("RK2 CUDA C Driver", root_region="Total")
    call profiler_end()
...
contains
            ...
            do j = G%jsd, G%jed
                do i = G%isd, G%ied
                    h0(i,j,k) = total_depth/real(GV%ke,dp) + &
                        ...
                        cos(real(j-1,dp)/real(G%nj,dp)*PI)*exp(-real(k,dp)/20.0_dp)
                    h(i,j,k) = h0(i,j,k)
    ...
    subroutine compute_transports_cuda(u_d, v_d, h_d, uh_d, vh_d, dyCu_d, dxCv_d, G, GV)
        ...
        do k = 1, GV%ke
            do j = jsd, jed
                do i = isd, ied
                    uh_d(i,j,k) = u_d(i,j,k)*0.5_dp*(h_d(i,j,k)+h_d(min(i+1,ied),j,k))*dyCu_d(i,j)
                    vh_d(i,j,k) = v_d(i,j,k)*0.5_dp*(h_d(i,j,k)+h_d(i,min(j+1,jed),k))*dxCv_d(i,j)
                end do
            end do
        end do
    ...
    subroutine velocity_update_cuda(vel_out_d, vel_in_d, accel_d, diff_d, dt_scale, &
        ...
        do k = 1, nk
            do j = js, je
                do i = is, ie
                    vel_out_d(i,j,k) = vel_in_d(i,j,k) + dt_scale*(accel_d(i,j,k) + diff_d(i,j,k))
                end do
            end do
        end do
    ...
    subroutine verify_state(h_init, h_final, u, v, eta, G, GV)
        type(ocean_grid_type), intent(in) :: G
        type(verticalGrid_type), intent(in) :: GV
        real(dp), dimension(G%isd:G%ied, G%jsd:G%jed, GV%ke), intent(in) :: h_init, h_final, u, v
        real(dp), dimension(G%isd:G%ied, G%jsd:G%jed), intent(in) :: eta
        real(dp) :: mass_init, mass_final, rel_error, max_u, max_v, max_eta, ke_total
        integer :: i, j, k
        mass_init = 0.0_dp; mass_final = 0.0_dp
        max_u = 0.0_dp; max_v = 0.0_dp; max_eta = 0.0_dp; ke_total = 0.0_dp
        do k = 1, GV%ke
            do j = G%jsc, G%jec
                do i = G%isc, G%iec
                    ...
                    ke_total = ke_total + 0.5_dp*(u(i,j,k)**2+v(i,j,k)**2)*h_final(i,j,k)*G%areaT(i,j)
                end do
            end do
        end do
        do j = G%jsc, G%jec
    ...
    subroutine check_bt_cfl(dt, nsteps, dx_km, depth)
        ...
        integer :: min_nsteps
        dx_m = dx_km*1000.0_dp; dtbt = dt/real(nsteps,dp)
        c_grav = sqrt(G_EARTH*depth); cfl = c_grav*dtbt/dx_m; cfl_2d = cfl*sqrt(2.0_dp)
    ...
    subroutine hor_visc_init_cuda_from_grid(CS, G, nk, Kh)
        ...
        real(dp), allocatable :: DY_dxT(:,:), DX_dyT(:,:), DY_dxBu(:,:), DX_dyBu(:,:)
        ...
        do j = jsd, jed
            do i = isd, ied
                ...
                dx2h(i,j) = G%dxT(i,j)**2; dy2h(i,j) = G%dyT(i,j)**2
                dx2q(i,j) = G%dxBu(i,j)**2; dy2q(i,j) = G%dyBu(i,j)**2
                reduction_xx(i,j) = 1.0_dp; reduction_xy(i,j) = 1.0_dp
            end do
        end do
        h_neglect = 1.0e-3_dp
        call hor_visc_init_cuda(CS, isd, ied, jsd, jed, G%isc, G%iec, G%jsc, G%jec, nk, &
            Kh, h_neglect, DY_dxT, DX_dyT, DY_dxBu, DX_dyBu, &
            G%IdyCu, G%IdxCu, G%IdyCv, G%IdxCv, G%IareaCu, G%IareaCv, &
            G%mask2dT, G%mask2dBu, reduction_xx, reduction_xy, dy2h, dx2h, dy2q, dx2q)
        deallocate(DY_dxT, DX_dyT, DY_dxBu, DX_dyBu, reduction_xx, reduction_xy)
    ...
    subroutine barotropic_init_cuda_from_grid(CS, G, dt, nstep)
        ...
        real(dp), allocatable :: f_4_u(:,:,:), f_4_v(:,:,:), bt_rem_u(:,:), bt_rem_v(:,:)
        real(dp) :: depth, bebt, f0
        integer :: i, j, isd, ied, jsd, jed
        isd = G%isd; ied = G%ied; jsd = G%jsd; jed = G%jed
        depth = 4000.0_dp; bebt = 0.2_dp
        ...
        do j = jsd, jed
            do i = isd, ied
                Datu(i,j) = depth*G%dyCu(i,j); Datv(i,j) = depth*G%dxCv(i,j)
                gtot_E(i,j) = G_EARTH; gtot_W(i,j) = G_EARTH
                gtot_N(i,j) = G_EARTH; gtot_S(i,j) = G_EARTH
                bt_rem_u(i,j) = 0.999_dp; bt_rem_v(i,j) = 0.999_dp
                f0 = G%CoriolisBu(i,j)
                f_4_u(:,i,j) = 0.25_dp*f0; f_4_v(:,i,j) = 0.25_dp*f0
            end do
        end do
        call barotropic_init_cuda(CS, isd, ied, jsd, jed, G%isc, G%iec, G%jsc, G%jec, &
            nstep, dt, bebt, 0, Datu, Datv, gtot_E, gtot_W, gtot_N, gtot_S, &
            f_4_u, f_4_v, bt_rem_u, bt_rem_v, G%IareaT, G%IdxCu, G%IdyCv)
        deallocate(Datu, Datv, gtot_E, gtot_W, gtot_N, gtot_S, f_4_u, f_4_v, bt_rem_u, bt_rem_v)
    ...
    subroutine init_ocean_grid_cuda(G, ni, nj, nk, dx_km, lat_deg)
        ...
        G%isd = 1; G%ied = ni+2; G%jsd = 1; G%jed = nj+2
        G%isc = G%isd+3; G%iec = G%ied-3; G%jsc = G%jsd+3; G%jec = G%jed-3
        dx_m = dx_km*1000.0_dp; G%dx = dx_m; G%dy = dx_m
        allocate(G%IareaT(G%isd:G%ied,G%jsd:G%jed), G%areaT(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%dxT(G%isd:G%ied,G%jsd:G%jed), G%dyT(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IdxT(G%isd:G%ied,G%jsd:G%jed), G%IdyT(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%dxCu(G%isd:G%ied,G%jsd:G%jed), G%dyCu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%dy_Cu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IdxCu(G%isd:G%ied,G%jsd:G%jed), G%IdyCu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%dxCv(G%isd:G%ied,G%jsd:G%jed), G%dyCv(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IdxCv(G%isd:G%ied,G%jsd:G%jed), G%IdyCv(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IareaBu(G%isd:G%ied,G%jsd:G%jed), G%areaBu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%CoriolisBu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IareaCu(G%isd:G%ied,G%jsd:G%jed), G%IareaCv(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%dxBu(G%isd:G%ied,G%jsd:G%jed), G%dyBu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%IdxBu(G%isd:G%ied,G%jsd:G%jed), G%IdyBu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%bathyT(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%mask2dT(G%isd:G%ied,G%jsd:G%jed), G%mask2dBu(G%isd:G%ied,G%jsd:G%jed))
        allocate(G%mask2dCu(G%isd:G%ied,G%jsd:G%jed), G%mask2dCv(G%isd:G%ied,G%jsd:G%jed))
        f0 = 2.0_dp*OMEGA*sin(lat_deg*PI/180.0_dp)
        beta = 2.0_dp*OMEGA*cos(lat_deg*PI/180.0_dp)/EARTH_RADIUS
        do j = G%jsd, G%jed
            do i = G%isd, G%ied
                G%areaT(i,j) = dx_m*dx_m; G%IareaT(i,j) = 1.0_dp/(dx_m*dx_m)
                G%dxT(i,j) = dx_m; G%dyT(i,j) = dx_m
                G%IdxT(i,j) = 1.0_dp/dx_m; G%IdyT(i,j) = 1.0_dp/dx_m
                G%dxCu(i,j) = dx_m; G%dyCu(i,j) = dx_m; G%dy_Cu(i,j) = dx_m
                G%IdxCu(i,j) = 1.0_dp/dx_m; G%IdyCu(i,j) = 1.0_dp/dx_m
                G%dxCv(i,j) = dx_m; G%dyCv(i,j) = dx_m
                G%IdxCv(i,j) = 1.0_dp/dx_m; G%IdyCv(i,j) = 1.0_dp/dx_m
                G%areaBu(i,j) = dx_m*dx_m; G%IareaBu(i,j) = 1.0_dp/(dx_m*dx_m)
                G%IareaCu(i,j) = 1.0_dp/(dx_m*dx_m); G%IareaCv(i,j) = 1.0_dp/(dx_m*dx_m)
                G%dxBu(i,j) = dx_m; G%dyBu(i,j) = dx_m
                G%IdxBu(i,j) = 1.0_dp/dx_m; G%IdyBu(i,j) = 1.0_dp/dx_m
                G%mask2dT(i,j) = 1.0_dp; G%mask2dBu(i,j) = 1.0_dp
                G%mask2dCu(i,j) = 1.0_dp; G%mask2dCv(i,j) = 1.0_dp

!> MPI-aware RK2 driver for MOM6 miniapps (pure CPU serial)
!!
!! Based on rk2_mpi_omp_driver.F90 but with all GPU directives removed.
!! Physics modules use the _serial variants; halo exchange uses the pure-CPU
!! mom6_mpi_halo from src/common/.
!!
!! Usage: mpirun -np N rk2_mpi_serial_driver ni nj nk niter bt_nsteps [npes_x npes_y]
!!        npes_x * npes_y must equal N
!!        If npes_x/npes_y omitted, auto-decomposed via MPI_Dims_create
!!
program rk2_mpi_serial_driver
    use mpi
    ...
    use mom6_continuity_serial, only: continuity_CS, continuity_init, continuity_PPM, continuity_end
    use mom6_coriolis_serial, only: coriolis_CS, coriolis_init, CorAdCalc, coriolis_end, &
                             SADOURNY75_ENERGY
    use mom6_barotropic_serial, only: barotropic_CS, barotropic_init, barotropic_end, &
                               btstep_init_state, btstep_do_step, btstep_get_output
    use mom6_vert_visc_serial, only: vert_visc_CS, vert_visc_init, &
                              vert_visc_coef_apply, vert_visc_end
    use mom6_hor_visc_serial, only: hor_visc_CS, hor_visc_init, hor_visc, hor_visc_end
    ...
    if (MD%rank == 0) then
        print '(A)', '=================================================='
        print '(A)', 'MOM6 Split RK2 MPI Driver (Serial CPU)'
        print '(A)', '=================================================='
    ...
    if (MD%rank == 0) then
        ...
        print '(A)', ''
        print '(A)', 'Running split RK2 time-stepping (MPI + Serial CPU)...'
        print '(A)', ''
    end if
    ...
    if (MD%rank == 0) then
        call profiler_report("RK2 MPI Serial Driver", root_region="Total")
    end if
    call profiler_end()
...
end program rk2_mpi_serial_driver
